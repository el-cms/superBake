...
// Edit (Only on view)
	if ($this->canDo('edit') && $viewIsAnItem) {
		$current_toolbar[] = "<?php echo \$this->Html->Link(" . $this->iString("Edit this $singularHumanName") . "," . $this->url('edit', $pluralVar, null, "\${$singularVar}['{$modelClass}']['{$primaryKey}']") . ");?>";
	}

// (Only on view)
	if ($this->canDo('delete') && $viewIsAnItem) {
		$current_toolbar[] = "<?php echo \$this->Form->postLink(" . $this->iString("Delete this $singularHumanName") . ", " . $this->url('delete', null, null, "\${$singularVar}['{$modelClass}']['{$primaryKey}']") . ", array('confirm' => __('Are you sure you want to delete \"%s\"?', \${$singularVar}['{$modelClass}']['{$displayField}']), 'title'=>__('Delete this entry')); ?>";
	}
