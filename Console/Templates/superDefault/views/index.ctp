...
$haveActions=false;
// View
if($this->canDo('view') === true){
	$haveActions=true;
	$item_actions.="\t\t\t<?php echo \$this->Html->link(" . $this->iString('View') . ", array('action' => 'view', \${$singularVar}['{$modelClass}']['{$primaryKey}'])); ?>\n";
}
// Edit
if($this->canDo('edit') === true){
	$haveActions=true;
	$item_actions.="\t\t\t<?php echo \$this->Html->link(" . $this->iString('Edit') . ", array('action' => 'edit', \${$singularVar}['{$modelClass}']['{$primaryKey}'])); ?>\n";
}
// Delete
if($this->canDo('delete') === true){
	$haveActions=true;
	"\t\t\t<?php echo \$this->Form->postLink(" . $this->iString('Delete') . ", array('action' => 'delete', \${$singularVar}['{$modelClass}']['{$primaryKey}']), null, __('Are you sure you want to delete # %s?', \${$singularVar}['{$modelClass}']['{$primaryKey}'])); ?>\n";
}
...
<div class="<?php echo $pluralVar; ?> index">
	<table cellpadding="0" cellspacing="0">
		<tr>
			...
			endforeach;
			if ($hideActionsList === false && $haveActions === true):
			?>
