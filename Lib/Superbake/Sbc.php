		...
		if (empty($default)) {
			...
			foreach ($default as $k => $v) {
				...
				if (is_numeric($k)) {
					// Search in defined
					if (!is_array($v)) {
						// We remove it as we will take the value from the other array
						if (in_array($v, $defined)) {
							unset($defined[array_search($v, $defined)]);
						}
						$finalArray[] = $v;
					} else {
						$this->log("Something is wrong with you default values: you should not define arrays without a named key (Key nb: $k)", 'error');
					}
				} else {
					// Check in defined
			...
			// Values defined alone. What must we do with it ?
			if ($keep === true) {
				foreach ($defined as $k => $v) {
					if (is_numeric($k)) {
						if (!is_array($v)) {
							...
							$this->log("Something is wrong with defined values: you should not define arrays without a named key (Key nb: $k))", 'error');
						}
					} else {
						$finalArray[$k] = $v;
					}
