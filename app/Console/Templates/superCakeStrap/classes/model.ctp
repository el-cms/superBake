 ...
 * ========================
 * $snippets : an array of snippetName=>snippet::path
 * 			SnippetName isn't used in generation, but allows you to organize your config file.
 * $displayField
 * 
 * ---
...
/* -----------------------------------------------------------------------------
 * Model options, from config file.
 * These options are the default options, If you want to handle more for your snippets,
 * check yours in your snippets files
 */
// Snippet files
if (!isset($snippets)) {
	$snippets = array();
...
* 
* @copyright     Copyright 2012-<?php echo date('Y') ?>, <?php echo $projectConfig['general']['editorName'] ?> (<?php echo $projectConfig['general']['editorWebsite'] ?>)
* @author        <?php echo $projectConfig['general']['editorName'] ?> <<?php echo $projectConfig['general']['editorEmail'] ?>>
* @link          <?php echo $projectConfig['general']['editorWebsite'] ?> <?php echo $projectConfig['general']['editorWebsiteName'] . "\n" ?>
* @package       <?php echo $projectConfig['general']['basePackage'] ?>/<?php echo $plugin . "\n" ?>
*
<?php
$licenseTemplate = dirname(dirname(__FILE__)) . DS . 'common' . DS . 'licenses' . DS . $projectConfig['general']['editorLicenseTemplate'] . '.ctp';
...
foreach (array('hasOne', 'belongsTo') as $assocType):
	if (!empty($associations[$assocType])):
		...
		foreach ($associations[$assocType] as $i => $relation):
			$relationPlugin = $this->getModelPlugin($relation['className']);
			if (!is_null($relationPlugin)) {
...
//die(var_dump($snippets));
foreach ($snippets as $snippet => $v) {
