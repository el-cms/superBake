<?php
/**
 * Index view for a news list. This view has many configuration options.
 * 
 ...
 * - contentField : string, default 'text'. Name of the content field
 * - titleField: string, default 'title'. Name of the title field.
...
// Content field
if (!isset($contentField) || !is_array($contentField)) {
	$contentField = 'text';
}
// Title field
if (!isset($titleField)) {
...
if(!isset($categoryField)){
	$categoryField=  Inflector::underscore($singularVar).'_id';
}

// Adding title and content fields to hidden fields list
$hiddenFields[] = $titleField;
$hiddenFields[] = $contentField;
//$hiddenFields[] = $licenseField;
...
if ($noToolbar === false) {
	...
	include(dirname(__FILE__) . DS . '..' . DS . 'common' . DS . 'toolbar_buttons.ctp');
	echo "</div>\n";
}
...
<div class="toolbar">
	<strong><?php echo "<?php echo __('Order by:');?>"; ?></strong>
	<div class="btn-group">
		...
		foreach ($fields as $field) {
			...
			if (!in_array($field, $hiddenFields)) {
				if (in_array($field, $sortableFields)) {
					...
					echo "\$icon='<i class=\"icon-sort-by-alphabet\"></i> ';\n";
					echo "\t\t\t\t\telseif(\$this->Paginator->sortDir() == 'asc' && \$this->Paginator->sortKey() == '$field') :\n";
					echo "\$icon='<i class=\"icon-sort-by-alphabet-alt\"></i> ';\n";
					echo "else:\n";
					echo "\$icon='';\n";
					echo "\t\t\t\t\tendif;\n";
					echo "\t\t\t\t\techo \$this->Paginator->sort('{$field}', \$icon . " . $this->display(Inflector::humanize($field)) . ", array('class'=>'btn btn-primary btn-xs', 'escape'=>false )); ?>\n";
					//$i++;
				} 
//				else {
//					echo "\t\t\t\t<a class=\"btn btn-default btn-xs\" disabled=\"disabled\"><?php echo " . $this->display(Inflector::humanize($field)) . "; ? ></a>\n";
//				}
			}
		}
		?>
	...
	<div class="panel">
		<div class="panel-heading">
			<h3 class="panel-title">
				<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#collapse<?php echo '<?php echo $i ?>'; ?>">
					<?php echo "<?php echo h(\${$singularVar}['{$modelClass}']['$titleField']); ?>"; ?>
				</a>
			</h3>
		</div>
		<div id="collapse<?php echo '<?php echo $i ?>'; ?>" class="panel-collapse collapse<?php echo "<?php echo \$in;?>"; ?>">
				...
				<div class="row">
					...
					<div class="col-lg-9">
						...
						<p>
							<?php 
							//License field
							if(in_array($licenseField, $fields)){
								echo $licenseField;
								echo "<i class=\"icon-circle-blank\">&copy;</i> <?php echo \${$singularVar}['{$modelClass}']['$licenseField']; ?>\n";
							}
							//Category field
							if(in_array($licenseField, $fields)){
								echo $licenseField;
									...
									foreach ($associations['belongsTo'] as $alias => $details) {
										if ($field === $details['foreignKey']) {
											...
											if ($this->actionable('view', $details['controller'])) {
												...
												echo "\t\t\t\t\t<dd><?php echo \${$singularVar}['{$alias}']['{$details['displayField']}']; ?></dd>\n";
											}
											break;
										}
									}
...
//									if ($i == 0 && $item_view == 1) {
//										echo "\t\t\t\t<dt>$field:</dt>\n";
//										echo "<dd>\n<?php echo \$this->Html->link(" . $this->display("\${$singularVar}['{$modelClass}']['{$primaryKey}']") . ", " . $this->url('view', null, "\${$singularVar}['{$modelClass}']['{$primaryKey}']") . ") ? >\n</dd>\n";
//										$i++;
//									} else {
										echo "\t\t\t\t<dt>$field:</dt>\n";
